...
<div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="max-w-xl mx-auto bg-white rounded-xl shadow-2xl overflow-hidden">
        <div class="p-8 sm:p-10">
            <h1 class="text-4xl font-bold text-gray-900 mb-2 text-center">Create New Product</h1>
            <p class="text-center text-gray-500 mb-8">Fill out the details below to add a new product to your catalog.</p>
            <form action="{{ route('products.store') }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name') }}" placeholder="e.g., Organic Coffee Beans" class="w-full border border-gray-300 rounded-lg shadow-sm px-4 py-2.5 bg-gray-50 text-gray-900 placeholder-gray-400 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent @error('name') border-red-500 @enderror">
                    @error('name') 
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p> 
                    @enderror
                </div>
                
                {{-- Product Description Field with Icon --}}
                <div>
